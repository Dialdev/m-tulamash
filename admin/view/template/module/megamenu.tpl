...
$(document).ready(function() {
	...
	$('#links_autocomplete').autocomplete({
		...
		source: function(request, response) {		
			$.ajax({
				url: 'index.php?route=module/megamenu/autocomplete&token=<?php echo $_GET['token']; ?>&filter_name=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					response($.map(json, function(item) {
						return {
	...
	function menu_updatesort(jsonstring) {
	     $.post("index.php?route=module/megamenu&token=<?php echo $_GET['token']; ?>&module_id=<?php echo $active_module_id; ?>", {
