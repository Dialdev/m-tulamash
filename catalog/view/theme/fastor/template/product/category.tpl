...
<div class="product-list active"> 
	...
	<div>
		<div class="row table-td">
			...
			<div class="name-actions col-sm-6">
			...
			</div>			<div class="desc col-sm-2">
				<?/*<div class="description"><?php echo $product['description']; ?></div>*/?>
				<a href="<?php echo $product['href']; ?>" style=" color:#555;   border: 1px solid #777;    padding: 5px 15px;    margin: 10px;    display: inline-block;    ;">Подробнее</a>
			</div>

			<div class="name-actions col-sm-2"><ul>
				     <?/*<?php if($theme_options->get( 'display_add_to_cart' ) != '0') { ?>
				          <?php $enquiry = false; if($config->get( 'product_blocks_module' ) != '') { $enquiry = $theme_options->productIsEnquiry($product['product_id']); }
				          if(is_array($enquiry)) { ?>
				          <li><a href="javascript:openPopup('<?php echo $enquiry['popup_module']; ?>', '<?php echo $product['product_id']; ?>')" data-toggle="tooltip" data-original-title="<?php echo $enquiry['block_name']; ?>"><i class="fa fa-question"></i></a></li>
			...
			<div class="name-actions col-sm-2">
											<button id="smcup-float-button-4" class="btn2 btn-primary " onclick="getOCwizardModal_smcup(4, 1)">Заказать товар</button>
